...
export class Fan {
  ...
  async setOnOffToggle({ device }: { device: irdevice & irDevicesConfig; }): Promise<void> {
    if (device.irfan?.on_off_toggle === undefined) {
      this.onOffToggle = false;
    } else {
      this.onOffToggle = device.irfan!.on_off_toggle;
    }
  }
